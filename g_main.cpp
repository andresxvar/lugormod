...
void QDECL G_Printf(const char *fmt, ...)
{
	...
	{
		if (g_nakenPassword.string[0] && Q_strncmp(text, "info: ", 6) == 0)
		{
		...
		else if (g_nakenPassword.string[0] && Q_strncmp(text, "tell: ", 6) == 0)
		{
			naken_queue(va("%%%s", text + 6));
		}
		else if (g_nakenPassword.string[0] && Q_strncmp(text, "sayteam: ", 9) == 0)
		{
			naken_queue(va("%%%s", text + 9));
		}
		else if (g_nakenPassword.string[0] && Q_strncmp(text, "ERROR: ", 7) == 0)
		{
...
void G_RegisterCvars(void)
{
	...
	//=========================================================
	trap_Cvar_Register(&Lugormod_Version, cvar, va("U#X %i.%i.%i.%i", verMajor, verMinor, verRev, verBuild), CVAR_ROM | CVAR_SERVERINFO);
...
void G_InitGame(int levelTime, int randomSeed, int restart)
{
	...
	if (g_gametype.integer == GT_HOLOCRON || g_gametype.integer == GT_REBORN || g_gametype.integer == GT_JEDIMASTER || HasSetSaberOnly() || AllForceDisabled(g_forcePowerDisable.integer))
	{
	...
	*/
	...
	if ((g_gametype.integer == GT_FFA || g_gametype.integer == GT_TEAM) && g_dontLoadNPC.integer)
	{ //Lugormod don't load waypoints
	...
	*/
	...
	if (g_gametype.integer == GT_JEDIMASTER && g_jmsaberreplace.integer && !G_Find(NULL, FOFS(classname), "info_jedimaster_start"))
	{
		int antlitems = 0;
...
void FindIntermissionPoint(void)
{
	...
	if ((g_gametype.integer == GT_SIEGE || g_gametype.integer == GT_BATTLE_GROUND) && level.intermissiontime && level.intermissiontime <= level.time && gSiegeRoundEnded)
	{
...
void LogExit(const char *string)
{
	...
	if (g_gametype.integer >= GT_TEAM && g_gametype.integer != GT_REBORN)
	{
...
void CheckVote(void)
{
	if (level.voteExecuteTime && level.voteExecuteTime < level.time)
	{
		...
		if (level.votingGametype)
		{
			...
			{ //If we're voting to a different game type, be sure to refresh all the map stuff
				...
				if (level.votingGametypeTo == GT_SIEGE || level.votingGametypeTo == GT_BATTLE_GROUND)
	...
	{
		if (g_voteFix.integer && level.voteYes > level.voteNo && Q_strncmp(level.voteString, "kick", 4) && Q_strncmp(level.voteString, "clientkick", 10))
		{
...
ContinueThink:
	*/
		...
		g_siegeLeaveCheck < level.time)
	{
		...
		if (G_CountHumanPlayers(-1) == 0 && g_allowVote.integer & (1 | 16))
		{
...
#define JETPACK_REFUEL_RATE 150 //seems fair
#ifdef LMD_NEW_JETPACK
			if (ent->client->jetPackTime > level.time)
			...
			else
			{
				...
				if (gameMode(GM_ALLWEAPONS) && (ent->client->ps.powerups[PW_REDFLAG] || ent->client->ps.powerups[PW_BLUEFLAG] || ent->client->ps.powerups[PW_NEUTRALFLAG]))
				{
					ent->client->jetPackDebRecharge = level.time;
				}
...
#define CLOAK_REFUEL_RATE 150 //seems fair
			if (ent->client->ps.powerups[PW_CLOAKED] &&
				...
				g_gametype.integer != GT_GHOST && !gameMode(GMF_CLOAKING))
