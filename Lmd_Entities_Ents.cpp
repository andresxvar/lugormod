...
char *fixpath(char *str)
{
	...
	for (; *h && h - str < MAX_STRING_CHARS && Q_strncmp(h, ".md3", 4);
		 h++)
	{
		*(t++) = *h;
	}
...
qboolean lmd_rentterminal_pay(gentity_t *self, int credits, gentity_t *activator)
{
	...
	if (self->genericValue2 > 0 && (sec + curSeconds) > self->genericValue2)
	{
		...
		Q_strcat(msg, sizeof(msg), va(".\n"
									  ...
									  (int)floor((float)((self->genericValue2 - curSeconds) / (float)self->genericValue1) * (float)self->count)));
		Disp(activator, msg);
		return qfalse;
	}
	...
	if (self->genericValue3 > 0 && (sec + curSeconds) < self->genericValue3)
	{
		...
		Q_strcat(msg, sizeof(msg), va(".\n"
									  ...
									  (int)floor((float)((self->genericValue3 - curSeconds) / (float)self->genericValue1) * (float)self->count)));
		Disp(activator, msg);
		return qfalse;
	}
...
void lmd_rentterminal_examine(gentity_t *self, gentity_t *activator)
{
	...
	Q_strcat(msg, sizeof(msg), va("^3The rent %s ^3expire if you die.\n"
								  "^3The terminal %s ^3more credits after the first payment before it expires.",
								  (self->spawnflags & 1) ? "^1will" : "^2will not", (self->spawnflags & 2) ? "^2accepts" : "^1does not accept"));
