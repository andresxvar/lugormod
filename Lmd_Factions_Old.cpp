...
DBSaveFileCallbackReturn_t *Factions_SaveMoreKeys(byte *structure, DBSaveFileCallbackReturn_t *arg, char *key, int keySze, char *value, int valueSze)
{
	...
	*/
	if (arg->func == 0)
	{
		...
		Q_strncpyz(value, va("%u \"%s\"", faction->Players.Player[arg->offset].playerId, faction->Players.Player[arg->offset].rankName), valueSze);
		arg->offset++;
		return arg;
	}
	else if (arg->func == 1)
	{
		...
		Q_strncpyz(value, va("\"%s\" \"%s\" %u %u", faction->Ranks.Rank[arg->offset].name, faction->Ranks.Rank[arg->offset].rankTag, faction->Ranks.Rank[arg->offset].rankLevel, faction->Ranks.Rank[arg->offset].auths), valueSze);
		arg->offset++;
		return arg;
	}
	else if (arg->func == 2)
	{
		...
		Q_strncpyz(value, va("\"%s\" %u \"%s\"", faction->Announcements.Message[arg->offset].title, faction->Announcements.Message[arg->offset].timeoutDay, faction->Announcements.Message[arg->offset].text), valueSze);
		arg->offset++;
		return arg;
	}
	return NULL;
		...
		else if (Q_stricmp(arg, "read") == 0)
		{
			...
			if (argc < 5 + (iArg == 1))
			{
				Disp(ent, va("^3Usage: %s <faction name> announcements read <announcement name>\n"
							 "^3This command lets you read faction-wide announcements.",
							 iArg ? "factionadmin edit" : "factions"));
				return;
			}
			//trap_Argv(4 + (iArg == 1), arg, sizeof(arg));
		...
		else if (Q_stricmp(arg, "delete") == 0)
		{
			...
			if (argc < 5 + (iArg == 1))
			{
				Disp(ent, va("^3Usage: %s <faction name> announcements delete <announcement name>\n"
							 "^3This command will delete an announcement.",
							 iArg ? "factionadmin edit" : "factions"));
				return;
			}
			trap_Argv(4 + (iArg == 1), arg, sizeof(arg));
