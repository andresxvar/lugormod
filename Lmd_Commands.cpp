...
gentity_t *AimTarget(gentity_t *ent, int length)
{
	...
	if (tEnt && tEnt->inuse && tEnt->client && tEnt->client->pers.connected == CON_CONNECTED)
	{
		return tEnt;
	}
	return NULL;
...
gentity_t *HurlItem(gentity_t *ent, const char *name)
{
	...
	if (it->giType == IT_WEAPON)
	{
		...
		if (it->giTag != WP_BOWCASTER && it->giTag != WP_DET_PACK && it->giTag != WP_THERMAL && it->giTag != WP_TRIP_MINE)
		{
...
void Cmd_emote_f(gentity_t *ent, int iArg)
{
	...
	if (ent->client->ps.weapon == WP_SABER &&
		!((ent->client->ps.saberHolstered != 0) ^ (emotes[i].animStart == BOTH_TAVION_SWORDPOWER)))
	{
		Cmd_ToggleSaber_f(ent);
	}
	...
	if ((SETANIM_TORSO & emotes[i].setanim) && ent->client->ps.torsoTimer < 500)
	{
		ent->client->ps.torsoTimer = 500;
	}

	if ((SETANIM_LEGS & emotes[i].setanim) && ent->client->ps.legsTimer < 500)
	{
...
void Cmd_Ionlyduel_f(gentity_t *ent, int iArg)
{
	...
	if (ent->client->ps.weaponTime > 0 || ent->client->ps.weapon != WP_SABER || ent->client->ps.m_iVehicleNum || ent->client->pers.Lmd.persistantFlags & SPF_IONLYDUEL || duelInProgress(&ent->client->ps) || ent->client->ps.forceHandExtend != HANDEXTEND_NONE || BG_HasYsalamiri(g_gametype.integer, &ent->client->ps))
	{
		return;
	}
...
void Cmd_DropStuff_f(gentity_t *ent, int iArg)
{
	...
	case 1:
		...
		ent->client->ps.stats[STAT_HOLDABLE_ITEMS] &= ~(1 << hi);
