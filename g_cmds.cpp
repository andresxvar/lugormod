...
void SetTeam(gentity_t *ent, char *s)
{
	...
	else if (g_gametype.integer >= GT_TEAM)
	{
		...
		if (g_teamForceBalance.integer && !g_jediVmerc.integer && !gameMode(GM_ALLWEAPONS))
		{
	...
	else if ((g_gametype.integer == GT_POWERDUEL) && (level.numPlayingClients >= 3 || G_PowerDuelCheckFail(ent)))
	{
		team = TEAM_SPECTATOR;
	}
...
void changeSiegeClass(gentity_t *ent, char *className)
{
		...
		classnumber > 0)
	{
		...
		{
			Q_strncpyz(className, stm->classes[classnumber]->name, 64); //sizeof(className));
...
void displayClassesForTeam(gentity_t *ent)
{
...
void Cmd_GameCommand_f(gentity_t *ent)
{
	...
	{ //Ufo: was MAX_CLIENTS before, hence could crash a server with lower slots count
		return;
	}
...
void CallVote(gentity_t *ent, char *arg1, char *arg2)
{
	...
	if (ent && MAX_VOTE_COUNT && ent->client->pers.voteCount >= MAX_VOTE_COUNT && !Auths_PlayerHasAuthFlag(ent, AUTH_FULLVOTING))
	{
	...
	if (!Q_stricmp(arg1, "map_restart") && checkVoteEnabled(ent, VOTE_MAPRESTART))
	{
	}
	else if (!Q_stricmp(arg1, "nextmap") && checkVoteEnabled(ent, VOTE_NEXTMAP))
	{
	}
	else if (!Q_stricmp(arg1, "map") && checkVoteEnabled(ent, VOTE_MAP))
	{
	}
	else if (!Q_stricmp(arg1, "g_gametype") && checkVoteEnabled(ent, VOTE_GGAMETYPE))
	{
	}
	else if (!Q_stricmp(arg1, "kick") && checkVoteEnabled(ent, VOTE_KICK))
	{
	}
	else if (!Q_stricmp(arg1, "clientkick") && checkVoteEnabled(ent, VOTE_CLIENTKICK))
	{
	}
	else if (!Q_stricmp(arg1, "g_doWarmup") && checkVoteEnabled(ent, VOTE_GDOWARMUP))
	{
	}
	else if (!Q_stricmp(arg1, "timelimit") && checkVoteEnabled(ent, VOTE_TIMELIMIT))
	{
	}
	else if (!Q_stricmp(arg1, "fraglimit") && checkVoteEnabled(ent, VOTE_FRAGLIMIT))
	{
	}
	else if (!Q_stricmp(arg1, "g_gameMode") && checkVoteEnabled(ent, VOTE_GGAMEMODE))
	{
	...
	if (!Q_stricmp(arg1, "g_gametype"))
	{
		...
		if (i == GT_JEDIMASTER || i >= GT_BATTLE_GROUND)
		{
...
qboolean G_OtherPlayersDueling(void)
{
	...
	while (i < MAX_CLIENTS)
	{
		...
		if (ent && ent->inuse && ent->client && duelInProgress(&ent->client->ps))
		{
			return qtrue;
		}
		i++;
