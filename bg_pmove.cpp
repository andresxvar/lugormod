...
static void PM_Friction(void)
{
	...
	if (pEnt && pEnt->s.NPC_class == CLASS_VEHICLE && pEnt->m_pVehicle && pEnt->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER && pm->ps->groundEntityNum < ENTITYNUM_NONE)
	{
		drop += 100;
	}
...
#endif
{
	...
	if (pm->ps->groundEntityNum == ENTITYNUM_NONE &&			  //in air
		...
		pm->ps->fd.forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0 //force-jump capable~
																  //RoboPhred
#ifndef LMD_NEW_JUMP
		&& (pm->ps->velocity[2] > 0 || pm->ps->fd.forcePowerLevel[FP_LEVITATION] == FORCE_LEVEL_5)
#endif
			) //going up
	{
		return qtrue;
...
qboolean PM_AdjustAngleForWallRun(playerState_t *ps, usercmd_t *ucmd, qboolean doMove)
{
	...
	{ //wall-running and not at end of anim
		...
		if (trace.fraction < 1.0f && (trace.plane.normal[2] >= 0.0f && trace.plane.normal[2] <= 0.4f /*MAX_WALL_RUN_Z_NORMAL*/))
		{ //still a wall there
			if ((ps->legsAnim) == BOTH_WALL_RUN_RIGHT)
			{
				ucmd->rightmove = 127;
...
static qboolean PM_CheckJump(void)
{
	...
	if (pm->ps->groundEntityNum != ENTITYNUM_NONE || (pm->ps->origin[2] < pm->ps->fd.forceJumpZStart && jumpLevel < FORCE_LEVEL_5))
	{
			...
			if (PM_ForceJumping())
			{
				if (pm->ps->velocity[2] > 0 || jumpLevel == FORCE_LEVEL_5)
				{
					...
					if ((curHeight <= forceJumpHeight[0] ||					 //still below minimum jump height
						...
						curHeight < forceJumpHeight[jumpLevel] &&
					...
					{								//can still go up
						...
						//Lugormod
						if (jumpLevel == FORCE_LEVEL_5 && pm->ps->groundEntityNum == ENTITYNUM_NONE)
						{
			 ...
			 BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION))
	{
		...
		{ //on the ground
			...
			if (anim != -1)
			{
				...
				{ //there is a wall there.. or hit a client
					if ((anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT && anim != BOTH_FORCEWALLRUNFLIP_START) || (wallNormal[2] >= 0.0f && wallNormal[2] <= 0.4f /*MAX_WALL_RUN_Z_NORMAL*/))
		...
		{ //in the air
			...
			*/
					 ...
					 && jumpLevel > FORCE_LEVEL_1 && PM_WalkableGroundDistance() <= 80		 //unfortunately we do not have a happy ground timer like SP (this would use up more bandwidth if we wanted prediction workign right), so we'll just use the actual ground distance.
			...
			{																				 //run up wall, flip backwards
				if (allowWallRuns)
				{
					...
					if (1) //sure, we have it! Because I SAID SO.
					{
					 ...
					 && BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION) && (pm->ps->origin[2] - pm->ps->fd.forceJumpZStart) < (forceJumpHeightMax[FORCE_LEVEL_3] - (BG_ForceWallJumpStrength() / 2.0f)) //can fit at least one more wall jump in (yes, using "magic numbers"... for now)
																																																								//&& (pm->ps->legsAnim == BOTH_JUMP1 || pm->ps->legsAnim == BOTH_INAIR1 ) )//not in a flip or spin or anything
			)
...
static qboolean PM_CheckJump(void)
{
	...
	if (pm->ps->groundEntityNum != ENTITYNUM_NONE || (pm->ps->origin[2] < pm->ps->fd.forceJumpZStart && pm->ps->fd.forcePowerLevel[FP_LEVITATION] < FORCE_LEVEL_5))
	{
		pm->ps->fd.forcePowersActive &= ~(1 << FP_LEVITATION);
	}
		...
		{ //can't do this in zero-G
			...
			{ //holding jump in air
				...
				{								//can still go up
					...
					//need to scale this down, start with height velocity (based on max force jump height) and scale down to regular jump vel
					//Lugormod
					if (pm->ps->fd.forcePowerLevel[FP_LEVITATION] == FORCE_LEVEL_5 && pm->ps->groundEntityNum == ENTITYNUM_NONE)
					{
			 ...
			 BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION))
	{
		...
		{ //in the air
			...
			//Ufo: restoring flipkicks
					 ...
					 && pm->ps->fd.forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1 && pm->ps->velocity[2] > 200 && PM_GroundDistance() <= 80 //unfortunately we do not have a happy ground timer like SP (this would use up more bandwidth if we wanted prediction workign right), so we'll just use the actual ground distance.
					 && !BG_InSpecialJump(pm->ps->legsAnim) && pm->ps->duelInProgress && g_entities[pm->ps->clientNum].client->Lmd.duel.duelType & DT_FULL_FORCE)
			{ //run up wall, flip backwards
				vec3_t fwd, traceto, mins, maxs, fwdAngles;
				trace_t trace;
				vec3_t idealNormal;
					 ...
					 && !BG_InSpecialJump(pm->ps->legsAnim) && pm->ps->duelInProgress && g_entities[pm->ps->clientNum].client->Lmd.duel.duelType & DT_FULL_FORCE)
			{
				...
				{ //there is a wall there
					...
					if (trace.entityNum < MAX_CLIENTS)
					{
					 ...
					 && pm->ps->fd.forceRageRecoveryTime < pm->cmd.serverTime										   //not in a force Rage recovery period
					 && pm->ps->fd.forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1 && PM_WalkableGroundDistance() <= 80 //unfortunately we do not have a happy ground timer like SP (this would use up more bandwidth if we wanted prediction workign right), so we'll just use the actual ground distance.
					 && (pm->ps->legsAnim == BOTH_JUMP1 || pm->ps->legsAnim == BOTH_INAIR1))						   //not in a flip or spin or anything
			{																										   //run up wall, flip backwards
				if (allowWallRuns)
				{
...
static void PM_AirMove(void)
{
	...
	{ //reduced air control while not jetting
		...
		if ( //authenticated(&g_entities[pm->ps->clientNum],
			//SUPER_JP_LEVEL)
			pm->ps->userInt1 && pm->gametype == GT_FFA
			//&& !gameMode(GM_ANY)
		)
		{
...
static int PM_TryRoll(void)
{
	...
	if (BG_SaberInAttack(pm->ps->saberMove) || BG_SaberInSpecialAttack(pm->ps->torsoAnim) || BG_SpinningSaberAnim(pm->ps->legsAnim) || PM_SaberInStart(pm->ps->saberMove))
	...
	if ((pm->ps->weapon != WP_SABER && pm->ps->weapon != WP_MELEE && pm->ps->weapon != WP_STUN_BATON) || //Lugormod
		PM_IsRocketTrooper() ||
		BG_HasYsalamiri(pm->gametype, pm->ps) ||
		!BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime, FP_LEVITATION))
...
qboolean PM_CheckDualForwardJumpDuck(void)
{
	...
	if (pm->ps->legsAnim == BOTH_JUMPATTACK6)
	{
		...
		if ((pm->ps->legsTimer >= 1450 && PM_AnimLength(0, BOTH_JUMPATTACK6) - pm->ps->legsTimer >= 400) || (pm->ps->legsTimer >= 400 && PM_AnimLength(0, BOTH_JUMPATTACK6) - pm->ps->legsTimer >= 1100))
			...
			{ //move me up so the bottom of my bbox will be where the trace ended, at least
				...
				{ //should never start in solid
					...
					{ //crap, no room to expand, so just crouch us
						if (pm->ps->legsAnim != BOTH_JUMPATTACK6 || pm->ps->legsTimer <= 200)
...
static void PM_SetBounds()
{
	...
	else
	{
		...
		//Lugormod fix this
		else if (pm->ps->legsTimer && (pm->ps->legsAnim == BOTH_MEDITATE || pm->ps->legsAnim == BOTH_CROUCH3 || pm->ps->legsAnim == BOTH_SIT6))
		{
			pm->maxs[2] = pm->ps->crouchheight; //CROUCH_MAXS_2;
			//pm->ps->viewheight = (int)((DEFAULT_VIEWHEIGHT - MINS_Z) * scaleh) + MINS_Z;
			pm->ps->viewheight = DEFAULT_VIEWHEIGHT;
...
#define SLOPERECALCVAR pm->ps->slopeRecalcTime //this is purely convenience
	...
	if ((legsAnim >= LEGS_LEFTUP1 && legsAnim <= LEGS_LEFTUP5) || (legsAnim >= LEGS_S1_LUP1 && legsAnim <= LEGS_S1_LUP5) || (legsAnim >= LEGS_S3_LUP1 && legsAnim <= LEGS_S3_LUP5) || (legsAnim >= LEGS_S4_LUP1 && legsAnim <= LEGS_S4_LUP5) || (legsAnim >= LEGS_S5_LUP1 && legsAnim <= LEGS_S5_LUP5))
	...
	else if ((legsAnim >= LEGS_RIGHTUP1 && legsAnim <= LEGS_RIGHTUP5) || (legsAnim >= LEGS_S1_RUP1 && legsAnim <= LEGS_S1_RUP5) || (legsAnim >= LEGS_S3_RUP1 && legsAnim <= LEGS_S3_RUP5) || (legsAnim >= LEGS_S4_RUP1 && legsAnim <= LEGS_S4_RUP5) || (legsAnim >= LEGS_S5_RUP1 && legsAnim <= LEGS_S5_RUP5))
...
int BG_VehTraceFromCamPos(trace_t *camTrace, bgEntity_t *bgEnt, const vec3_t entOrg, const vec3_t shotStart, const vec3_t end, vec3_t newEnd, vec3_t shotDir, float bestDist)
{
	...
	if (!camTrace->allsolid && !camTrace->startsolid && camTrace->fraction < 1.0f && (camTrace->fraction * MAX_XHAIR_DIST_ACCURACY) > minAutoAimDist && ((camTrace->fraction * MAX_XHAIR_DIST_ACCURACY) - Distance(entOrg, camPos)) < bestDist)
	...
	if (vehicleRocketLock)
	{
		...
		{ //actually charging
			...
			{ //just make sure we have this veh info
				if (((pm->cmd.buttons & BUTTON_ATTACK) && g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID].fHoming && pm->ps->ammo[0] >= g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID].iAmmoPerShot) ||
					((pm->cmd.buttons & BUTTON_ALT_ATTACK) && g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[1].ID].fHoming && pm->ps->ammo[1] >= g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[1].ID].iAmmoPerShot))
...
static void PM_Weapon(void)
{
...
#else
			if (g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID].fHoming || g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[1].ID].fHoming)
	...
	{ //nothing else should be allowed to happen during this time, including weapon fire
		...
		case HANDEXTEND_FORCE_HOLD:
			...
			{ //lightning
				if (pm->ps->weapon == WP_MELEE && pm->ps->activeForcePass > FORCE_LEVEL_2)
				{ //2-handed lightning
	...
	*/
	...
	if (g_gametype.integer == GT_JEDIMASTER &&
		pm->ps->isJediMaster && pm->ps->emplacedIndex)
	{
	...
	else if (pm->ps->m_iVehicleNum && pm_entSelf->s.NPC_class == CLASS_VEHICLE)
	...
	{ //special anims for standard melee attacks
		...
		{ //if riding a vehicle don't do this stuff at all
			...
			else if (pm->debugMelee && (pm->cmd.buttons & BUTTON_ALT_ATTACK))
			{ //kicks
...
void PM_UpdateViewAngles(playerState_t *ps, const usercmd_t *cmd)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		if (pm_entVeh && pm_entVeh->m_pVehicle && pm_entVeh->m_pVehicle->m_pVehicleInfo && pm_entVeh->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER && (cmd->serverTime - pm_entVeh->playerState->hyperSpaceTime) >= HYPERSPACE_TIME)
								   ...
								   vec3_t ulAngles, vec3_t llAngles, vec3_t modelScale, float *tPitchAngle, float *tYawAngle, int *corrTime)
{
...
#else
	if (((!BG_FlippingAnim(cent->legsAnim) && !BG_SpinningSaberAnim(cent->legsAnim) && !BG_SpinningSaberAnim(cent->torsoAnim) && (cent->legsAnim) != (cent->torsoAnim)) //NOTE: presumes your legs & torso are on the same frame, though they *should* be because PM_SetAnimFinal tries to keep them in synch
		 ||
	...
	{
		if (!BG_FlippingAnim(cent->legsAnim) && !BG_SpinningSaberAnim(cent->legsAnim) && !BG_SpinningSaberAnim(cent->torsoAnim) && !BG_FlippingAnim(ciLegs) && !BG_SpinningSaberAnim(ciLegs) && !BG_SpinningSaberAnim(ciTorso))
		{
			doCorr = qtrue;
		}
	}
...
#endif
{
	...
	if ((pm->ps->legsAnim == BOTH_JUMPATTACK6 && pm->ps->saberMove == LS_JUMPATTACK_DUAL) || (pm->ps->legsAnim == BOTH_BUTTERFLY_FL1 && pm->ps->saberMove == LS_JUMPATTACK_STAFF_LEFT) || (pm->ps->legsAnim == BOTH_BUTTERFLY_FR1 && pm->ps->saberMove == LS_JUMPATTACK_STAFF_RIGHT) || (pm->ps->legsAnim == BOTH_BUTTERFLY_RIGHT && pm->ps->saberMove == LS_BUTTERFLY_RIGHT) || (pm->ps->legsAnim == BOTH_BUTTERFLY_LEFT && pm->ps->saberMove == LS_BUTTERFLY_LEFT))
	{
...
void PM_VehicleViewAngles(playerState_t *ps, bgEntity_t *veh, usercmd_t *ucmd)
{
	...
	if (veh->m_pVehicle->m_pPilot && veh->m_pVehicle->m_pPilot->s.number == ps->clientNum)
...
{
	if (pm->ps->legsAnim == BOTH_A7_SOULCAL && pm->ps->saberMove == LS_STAFF_SOULCAL)
	{ //forward spinning staff attack
		...
		if (pm->ps->legsTimer >= 2650 && pm->ps->legsTimer < 2850)
		{ //the jump
			if (pm->ps->groundEntityNum != ENTITYNUM_NONE)
			{ //still on ground?
				//jump!
...
void PmoveSingle(pmove_t *pmove)
{
	...
	else if (BG_SaberLockBreakAnim(pm->ps->legsAnim) || BG_SaberLockBreakAnim(pm->ps->torsoAnim) || pm->ps->saberLockTime >= pm->cmd.serverTime)
	{ //can't move or turn
		stiffenedUp = qtrue;
		PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
	}
	...
	{
		if ((pm->cmd.buttons & BUTTON_ATTACK) || (pm->cmd.buttons & BUTTON_ALT_ATTACK) || (pm->cmd.buttons & BUTTON_FORCEPOWER) || (pm->cmd.buttons & BUTTON_FORCEGRIP) || (pm->cmd.buttons & BUTTON_FORCE_LIGHTNING) || (pm->cmd.buttons & BUTTON_FORCE_DRAIN) || pm->cmd.upmove)
	...
	if (endAnim(pm->ps->torsoAnim) || endAnim(pm->ps->legsAnim))
	{
		stiffenedUp = qtrue;
		PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
	...
	else if ((emote = emoteAnim(pm->ps->torsoAnim))->name || (emote = emoteAnim(pm->ps->legsAnim))->name)
	{

		if (emote->animEnd != -1 && ((pm->cmd.buttons & BUTTON_ATTACK) || (pm->cmd.buttons & BUTTON_ALT_ATTACK) || (pm->cmd.buttons & BUTTON_FORCEPOWER) || (pm->cmd.buttons & BUTTON_FORCEGRIP) || (pm->cmd.buttons & BUTTON_FORCE_LIGHTNING) || (pm->cmd.buttons & BUTTON_FORCE_DRAIN) || pm->cmd.upmove))
		{ //End moves
		...
		else
		{
			if (emote->animEnd != -1)
			{
				if (emote->setanim & SETANIM_LEGS && pm->ps->legsTimer < 200)
				{
					pm->ps->legsTimer = 200;
				}
				if (emote->setanim & SETANIM_TORSO && pm->ps->torsoTimer < 200)
				{
					pm->ps->torsoTimer = 200;
				}
			}

			if (emote->setanim & SETANIM_LEGS && pm->ps->legsTimer > 0)
			{
				stiffenedUp = qtrue;
				PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
				pm->cmd.rightmove = 0;
...
#ifdef QAGAME
		else if (veh && veh->playerState && veh->playerState->vehTurnaroundIndex && veh->playerState->vehTurnaroundTime > pm->cmd.serverTime
				 //Lugormod only pilot
